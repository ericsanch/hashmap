...
class HashMap:
    ...
    def get(self, key):
        ...
        """
        position = self._hash_function(key) % self.capacity
        bucket = self._buckets[position]
        location = bucket.contains(key)
        if location:
    ...
    def resize_table(self, capacity):
        ...
        resized = []
        for i in range(capacity):
            resized.append(LinkedList())
        for j in range(self.capacity):
            while self._buckets[j].head is not None:
                element = self._buckets[j].head
                position = self._hash_function(element.key) % capacity
                bucket = resized[position]
                resized[position].add_front(element.key, element.key)
                self._buckets[j].remove(element.key)
        self.capacity = capacity
